	...
	def _toolbar_controller(viewfunc):
		def _toolbar_controlled(request, *args, **kwargs):
			...
			if type == "projects":
				...
				# Render toolbar
				toolbar = render_to_string('prototype/templates_toolbar.html', {'project': Project.objects.get_current(request), 'current_template': kwargs["template"]}, context_instance=RequestContext(request))
